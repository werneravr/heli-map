...
class SmartKMLManager {
  ...
  async processNewFiles() {
    ...
    // Save duplicates log (for rejected duplicates)
    if (duplicates.length > 0) {
      const duplicateLog = {
        rejected: duplicates.map(dup => ({
          ...dup,
          action: 'DELETED',
          timestamp: new Date().toISOString()
        })),
        summary: `${duplicates.length} duplicate files rejected and deleted`
      };
      fs.writeFileSync(this.duplicatesFile, JSON.stringify(duplicateLog, null, 2));
    }

    console.log(`📊 Processing complete:`);
    console.log(`  ✅ Processed: ${newFiles.length} new files`);
    console.log(`  📝 Renamed: ${renamed} files`);
    console.log(`  🗑️ Duplicates rejected: ${duplicates.length} files`);
...
Features:
  ...
  ✅ Identifies and rejects duplicates
